...
use spv\mi\core\classes\constants\CASTED;
use spv\mi\core\classes\Controller;
use spv\mi\core\classes\EInfo;
use spv\mi\core\classes\Request;
use spv\mi\core\classes\UrlMapper;
use spv\mi\windows\Routes\Docs\Wmv;
 ...
 */
 class Window extends Controller{
    ...
    /**
     * Defines a list of operations that must occur before a page  
     * is loaded
     *
     * @return void
     */
    ...
    final protected static function pushFormData(){
      if(isset($_SERVER['HTTP_REFERER'])){ 
        if($_SERVER['HTTP_REFERER'] != $_SERVER['REQUEST_URI']){
          ...
          $CSRF_TIME  = $_SESSION['CSRF_TIME'] ?? 0;

          $_SESSION[':FORM']['DATA']   = $data;
          $_SESSION[':FORM']['FILES']  = $file;
          $_SESSION[':FORM']['METHOD'] = $_SERVER['REQUEST_METHOD'];
          $_SESSION[':FORM']['URI']    = $_SERVER['REQUEST_URI'];
          $_SESSION[':FORM']['CSRF_REF'] = [
            'VALID'  => $CSRF_VALID, 
            'valid'  => $CSRF_VALID,
            'isValid'=> $CSRF_VALID,
            'time'   => $CSRF_TIME,
